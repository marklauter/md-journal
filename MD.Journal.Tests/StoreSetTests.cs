...
namespace MD.Journal.Tests
{
    public sealed class StoreSetTests
    {
        ...
        {
            var storeSet = new StoreSet<MemoryStore>(nameof(Constructor_Sets_Path));
        ...
        {
            var storeSet = new StoreSet<MemoryStore>(nameof(Indexer_Returns_Store));
            var store = storeSet[nameof(Indexer_Returns_Store)];
            Assert.NotNull(store);
            Assert.Equal(typeof(MemoryStore), store.GetType());
        }

        [Fact]
        public void Indexer_Returns_Same_Store_Twice()
        {
            var storeSet = new StoreSet<MemoryStore>(nameof(Indexer_Returns_Same_Store_Twice));
        ...
        {
            var storeSet = new StoreSet<MemoryStore>(nameof(Contains_Returns_True_When_Key_Present));
        ...
        {
            var storeSet = new StoreSet<MemoryStore>(nameof(Contains_Returns_False_When_Key_NotPresent));
